...
# file with the most allocated blocks. Use the span of its allocated physical
# extents when they cover at least 95% of the range, or its largest extent.
...
read -r extent_start extent_blocks extent_data_blocks < <(
    printf '%s\n' "$extent_output" | awk '
        /^[[:space:]]*[0-9]+:/ && $0 !~ /unwritten/ {
            ...
            sub(/:$/, "", end)
            blocks = end - start + 1
            if (!allocated || start < first) first = start
            if (!allocated || end > last) last = end
            allocated += blocks
            if (blocks > largest) {
                selected = start
                largest = blocks
        ...
        END {
            if (largest > 0) {
                span = last - first + 1
                if (allocated / span >= 0.95)
                    print first, span, allocated
                else
...
extent_coverage=$(awk -v data="$extent_data_blocks" -v total="$extent_blocks" 'BEGIN { printf "%.2f", 100 * data / total }')
echo "::notice title=${label} fio range::offset=${benchmark_offset} bytes, size=${benchmark_size} bytes, selected-file allocated coverage=${extent_coverage}%"
...
# Every profile is a single fio job. The hosted runner has 4 vCPUs and the
# backend's work pool is configured with 3 of them, so extra fio jobs would only
# contend with the implementation under test for the last CPU. Both labels run
# this identical shape, and all concurrency comes from iodepth, which loads the
# daemon exactly like a multi-job run of the same total depth.
echo "${label}: fio jobs: 1, concurrency from iodepth"
...
run_profile qd32 32
run_profile qd64 64
run_profile qd128 128
run_profile qd256 256
