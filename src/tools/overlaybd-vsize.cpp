...
static void usage() {
    static const char msg[] =
        ...
        "Usage: overlaybd-check <file | -r url >\n"
        "example:\n"
        "   ./overlaybd-check -r https://docker.io/v2/overlaybd/imgxxx/blobs/sha256:xxxxx\n";
...
int main(int argc, char **argv) {
    ...
    if (!remote) {
        // local file
        auto localfs = new_localfs_adaptor();
        auto file = localfs->open(argv[shift], O_RDONLY);
        auto zfile = new_tar_zfile_adaptor(file);
        auto lsmt_file = open_file_ro(zfile);
        printf("%ld\n", lsmt_file->lseek(0, SEEK_END));
        return 0;
    }
    ...
    auto remotefile = registryfs->open(url.c_str(), 0);
    if (remotefile == nullptr) {
        printf("failed to open registry blob.\n");
        exit(-1);
    }

    auto zfile = FileSystem::new_tar_zfile_adaptor(remotefile);
    auto lsmt_file = open_file_ro(zfile);
    printf("%ld\n", lsmt_file->lseek(0, SEEK_END) / 1024 / 1024 / 1024); // to GB
