...
private:
    ...
    PrefetcherImplV2* m_prefetcher;
};

class PrefetcherImplV2 : public Prefetcher {
public:
    explicit PrefetcherImplV2(const string& trace_file_path) {
        // Detect mode
        size_t file_size = 0;
        ...
        if (m_mode == Mode::Record) {
            ...
            close(lock_fd);
            auto th = photon::thread_create11(&PrefetcherImplV2::detect_lock, this);
    ...
    IFile* new_prefetch_file(IFile* src_file, uint32_t layer_index) override {
        return new PrefetchFileV2(src_file, layer_index, this);
    }
    ...
    void replay() override {
        ...
        for (int i = 0; i < REPLAY_CONCURRENCY; ++i) {
            auto th = photon::thread_create11(&PrefetcherImplV2::replay_worker_thread, this);
    ...
    int replay_worker_thread() {
        ...
        while (!m_replay_queue.empty() && !m_replay_stopped) {
            ...
            if (trace.op == PrefetcherImplV2::TraceOp::READ) {
...
private:
    ...
    friend LogBuffer& operator<<(LogBuffer& log, const PrefetcherImplV2::TraceFormat& f);
};

LogBuffer& operator<<(LogBuffer& log, const PrefetcherImplV2::TraceFormat& f) {
...
ssize_t PrefetchFileV2::pread(void* buf, size_t count, off_t offset) {
    ssize_t n_read = m_file->pread(buf, count, offset);
    if (n_read == (ssize_t) count && m_prefetcher->get_mode() == PrefetcherImplV2::Mode::Record) {
        m_prefetcher->record(PrefetcherImplV2::TraceOp::READ, m_layer_index, count, offset);
    }
    return n_read;
}

Prefetcher* new_prefetcher_v2(const string& trace_file_path) {
    return new PrefetcherImplV2(trace_file_path);
}
