...
public:
    void write(int, const char *, const char *) override {
        throttle_block();
    }
};
...
void LogFormatter::put_integer_hbo(ALogBuffer &buf, ALogInteger X) {
    ...
    auto begin = buf.ptr;
    do {
        put(buf, "0123456789ABCDEFH"[x & mask]);
    } while (x >>= shift);
...
public:
    ...
    int get_log_file_fd() override {
        return log_file_fd ? log_file_fd : -1;
    }
    ...
    int log_output_file_setting(const char *fn, uint64_t rotate_limit, int max_log_files) {
        const auto mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH;
...
ILogOutput *new_log_output_file(const char *fn, uint64_t rotate_limit, int max_log_files,
                                uint64_t throttle) {
...
int log_output_file(const char *fn, uint64_t rotate_limit, int max_log_files, uint64_t throttle) {
    if (default_log_output_file.log_output_file_setting(fn, rotate_limit, max_log_files) < 0)
...
LogBuffer &operator<<(LogBuffer &log, const Prologue &pro) {
    ...
    if (pro.level != ALOG_AUDIT) {
        char *file = (char *)pro.addr_file;
        int p = pro.len_file-1;
        while (p >= 0 && file[p] != '/') {
            p--;
        }

        log.printf(ALogString((char *)(pro.addr_file + p+1), pro.len_file-p-1), ':');
        log.printf(pro.line, '|');
