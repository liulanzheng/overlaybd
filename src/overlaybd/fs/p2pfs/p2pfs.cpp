...
public:
    ...
    virtual ~P2PFileSystem() override {
        delete m_refs;
        if (m_client_ownership) delete m_client;
    }

    void release(const std::string& filename) { m_refs->release(filename); }
    ...
    virtual IFile* open(const char* pathname, int flags) override {
        ...
        if (ref == nullptr)
            LOG_ERROR_RETURN(0, nullptr, "Failed to acquire file with ",
                             VALUE(pathname));
...
IFileSystem* new_p2pfs(RootSelector* root_selector, const NodeID& myid,
    ...
    if (myid.addr == 0 && myid.port == 0) {
        if (!balanced) {
            ...
            P2PClient* p2pClient =
                new_p2pclient(myid, root_selector, ttl, retry_time,
