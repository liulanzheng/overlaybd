...
## HOW TO USE:
# ABS_BUILD_NUMBER=`date +%Y%m%d%H%M%S`; rpm/t-storage-lsmd-build.sh <aone_place_holder_1> <aone_place_holder_2> 3.1.8 $ABS_BUILD_NUMBER aliyun
...
if [ -z "$TAG" ]; then
    ...
    echo "such as: ABS_BUILD_NUMBER=\`date +%Y%m%d%H%M%S\`; rpm/t-storage-lsmd-build.sh 1 1 3.1.8 \$ABS_BUILD_NUMBER aliyun"
    exit 1;
fi
